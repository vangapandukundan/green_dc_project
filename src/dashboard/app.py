...
# ── Guard: require train.py to run first or auto-train ────────
if results is None or df is None:
    with st.spinner("🤖 Initializing global deployment: Training Machine Learning models on the fly... (This will take ~30 seconds)"):
        import subprocess
    ...
    if results is None or df is None:
        st.error("❌ Auto-training failed. Please check the deployment logs.")
        st.stop()
    else:
        st.rerun()
